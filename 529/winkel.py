...
def lin_fit(Para, x):
    return 1/(Para[0]*np.sqrt(2*np.pi)) * np.exp(-Para[0]/2*(Para[2]*(x + Para[1]))**2)
...
lin_myodr = odr.ODR(lin_mydata, lin_model, beta0=[1, 0.3, 1], maxit=1000)
...
plt.errorbar(Winkel, Rate, yerr=RateErr, color='blue',capsize=4, elinewidth=1.3, capthick=0.8, linestyle='none')
# ax.legend()
plt.grid()
